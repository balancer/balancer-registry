...
describe('SmartOrderRouter', function(){
    ...
    before(async () => {
        ...
        // let liquidPools = await registry.getPairRegistry.call(MKR, WETH);
    });

    it('SimplifiedCalcSplit swapExactOut, input_amount = 100,000', async () => {
        // let pools = await registry.getPairRegistry.call(MKR, WETH);
        ...
        assert.equal(result['swaps'][0][2].toString(),"34681223095510744100000");
        assert.equal(result['swaps'][1][2].toString(),"26215324499553495700000");
        assert.equal(result['swaps'][2][2].toString(),"25939039858875108300000");
        assert.equal(result['swaps'][3][2].toString(),"13164412546060651900000");
